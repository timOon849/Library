...
namespace Library.Service
{
    public class ReaderService : IReadersService
    {
        ...
        {
            var rentHistory = await _context.RentHistory.Where(r => r.ID_Reader == ID_Reader && r.Date_End == null).ToListAsync();
            var bookNames = new List<string>();
            foreach (var rent in rentHistory)
            {
                var book = await _context.Book.FindAsync(rent.ID_Book); // Получение книги по ее ID
                if (book != null)
                {
